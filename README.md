...
flowchart TD
    ...
    BG --> READER --> LLM
    BG --> WRITER --> TARGET
    WRITER -.->|"fills field, then stops"| TARGET

    classDef ext stroke-dasharray: 5 5
    class LLM,TARGET,WEB ext
```

Dashed nodes are outside the trust boundary — pages and feeds this project does
not control. The writer's dashed edge is the safety property: it fills the
compose field and stops, and a human presses Post.
...
sequenceDiagram
    ...
    Note over B,Q: later — scheduler fires after a randomised delay
...
reader/writer tabs are gone, and the UI can say so immediately rather than after
a timeout.
...
**Why a request registry?** WebSockets have no request/response semantics. The
...
|---|---|
| Extension disconnects mid-request | `rejectAll` settles all in-flight promises with a disconnect error; the dashboard reports it instead of spinning |
...
| Duplicate reply after a reconnect | `resolve()` returns `false` for an already-settled id; the late answer is ignored |
| Backend restarts | Queue survives in SQLite; the in-memory scheduler does not, and resumes on next start |
| Writer selectors change | DOM probe fails, item moves to `needs_manual_post` for manual handling |
| Source URL redirects to a private host | Re-validated after redirect and rejected |
| Malformed JSON in a stored row | `safeParse` returns `null`; one bad row cannot crash the scheduler |
...
- **Human in the loop.** Auto-submit is off by default; the writer stops at a
  filled field.
...
| `requestRegistry` | 10 | Request/response over a socket that has neither: resolve, reject, timeout, duplicate replies after a reconnect, and `rejectAll` on disconnect leaving no stray timers |
| `queueService` | 9 | Category rotation — never two of a kind in a row, age order preserved within a category, nothing dropped or duplicated |
| `safeJson` | 7 | A malformed stored row returns `null` instead of taking down the scheduler; circular and BigInt values do not throw |
